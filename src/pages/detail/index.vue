...
onActivated(() => {
  movieStore.setMovie(route.query.id)
})
</script>
...
<template>
  <v-container class="h-screen pa-0">
    <v-toolbar class="bg-white" style="position: fixed;z-index: 999;">
      <v-btn class="hidden-xs-only" icon="mdi-arrow-left" @click="router.back"/>
    ...
    <v-container class="bg-white rounded-lg pb-12">
      ...
      <v-row no-gutters>
        <v-col class="py-2 pl-2">
          <v-img cover class="rounded-lg h-100" :src="movie.img"></v-img>
        </v-col>
        <v-col class="py-2">
          <v-card>
            ...
            <v-card-subtitle class="pt-1">{{ movie.enName }}</v-card-subtitle>
            <v-card-text class="pt-1">
              <v-chip label class="mr-2" v-for="type in movie.type" :key="type">
                {{ type }}
              </v-chip>
            </v-card-text>
            <v-card-text class="pt-1 pb-0">{{ movie.releaseDate }}</v-card-text>
            <v-card-text class="pt-1 pb-0">中国大陆上映</v-card-text>
            <v-card-text class="pt-1 pb-0">{{ movie.length }}分钟</v-card-text>
          </v-card>
        </v-col>
      </v-row>
      <!-- 评分 -->
            ...
            <v-row no-gutters>
              ...
              <v-col>
                <v-card-item>
                  ...
                  <v-card-subtitle>
                    <span class="me-1">1-27 · 上海</span>
                  </v-card-subtitle>
                </v-card-item>
                <v-card-text class="pb-0">
                  <div>非常好电影，使我大脑旋转</div>
                </v-card-text>
                <v-card-actions>
